...
from googlyeyes.amortized_inference.trainers.base_trainer import BaseTrainer
from googlyeyes.amortized_inference.nets import AmortizerForTrialData, RegressionForTrialData
from googlyeyes.amortized_inference.simulators import TypingSimulator
from googlyeyes.amortized_inference.datasets import TypingUserDataset, TypingTrainDataset, TypingValidDataset
from googlyeyes.amortized_inference.utils import ReplayBuffer, CosAnnealWR
from googlyeyes.amortized_inference.configs import default_typing_config


class TypingTrainer(BaseTrainer):
    def __init__(self, config=None, model='CRTypist_v1'):
        self.config = deepcopy(default_typing_config) if config is None else config
        self.model = model if model in ["CRTypist_v1", "CRTypist_v2"] else "CRTypist_v1"
        ...
        self.simulator = TypingSimulator(config=self.config["simulator"], model=self.model)
        self.user_dataset = TypingUserDataset()
        self.valid_dataset = TypingValidDataset(model=self.model)
        ...
        for x_col in ["age", "gender"]:
            for y_sb, y_col in zip(self.param_symbol, self.targeted_params):
                if x_col == "gender":
                    ...
                    plt.legend(custom_lines, [f"$r={pearson_r:.2f}, p={pearson_p:.3f}$"], handlelength=0.6,
                               handletextpad=0.4)
